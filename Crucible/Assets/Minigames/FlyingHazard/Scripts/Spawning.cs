...
public class Spawning : MonoBehaviour
{
    ...
    private IEnumerator spawn1(float interval, GameObject enemy){
        ...
        clone = Instantiate(enemy, new Vector3(Random.Range(-8.6f, 8.6f), -5f, 0f), Quaternion.identity);
    ...
    private IEnumerator spawn2(float interval, GameObject enemy){
        yield return new WaitForSeconds(interval);
        GameObject clone = Instantiate(enemy, new Vector3(Random.Range(-8.6f, 8.6f), 5f, 0f), Quaternion.identity);
        clone.AddComponent<Destroyer>();
